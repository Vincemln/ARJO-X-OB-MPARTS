...
function renderDashboard() {
  ...
  let dashboardHTML = `
    <div class="dashboard-panel" style="background: var(--bg-secondary); border-radius: 32px; padding: 40px; border: 1px solid var(--glass-border); box-shadow: 0 40px 100px rgba(0,0,0,0.5);">
      <div class="dashboard-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 40px; border-bottom: 1px solid var(--glass-border); padding-bottom: 20px;">
        <div>
          <h2 style="color: var(--accent-red); font-size: 2rem;"><i class="fas fa-tachometer-alt"></i> ${role.toUpperCase()} PANEL</h2>
  ...
  if (role === 'Super Admin') {
    dashboardHTML += `
      <div class="admin-section" style="background: rgba(255,255,255,0.03); padding: 25px; border-radius: 20px; border: 1px solid var(--glass-border);">
        <h3 style="color: var(--accent-red); margin-bottom: 20px;"><i class="fas fa-users-cog"></i> User Management</h3>
            ...
            <tbody>
              ${users.map(u => `
                <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
                  ...
                  <td style="padding: 12px;"><span style="font-size: 0.7rem; padding: 4px 8px; border-radius: 4px; background: ${u.role === 'Super Admin' ? 'var(--accent-red)' : u.role === 'Admin' ? '#3498db' : '#555'}; color: #000; font-weight: 700;">${u.role}</span></td>
  ...
  if (role === 'Admin' || role === 'Super Admin') {
    dashboardHTML += `
      <div class="admin-section" style="background: rgba(255,255,255,0.03); padding: 25px; border-radius: 20px; border: 1px solid var(--glass-border);">
        <h3 style="color: var(--accent-red); margin-bottom: 20px;"><i class="fas fa-boxes"></i> Product Management</h3>
    ...
    dashboardHTML += `
      ...
      <div class="admin-section" style="background: rgba(255,255,255,0.03); padding: 25px; border-radius: 20px; border: 1px solid var(--glass-border);">
        ...
        <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 20px;">
          <div style="width: 60px; height: 60px; border-radius: 50%; background: linear-gradient(135deg, var(--accent-red), #ff6b35); display: flex; align-items: center; justify-content: center; font-size: 1.6rem; font-weight: 800; color: #000; flex-shrink: 0;">
...
function attachDashboardEvents() {
  ...
  document.getElementById('syncInventoryBtn')?.addEventListener('click', () => {
    ...
    status.innerHTML = '<span style="color: var(--accent-red);">⚡ Connecting to Warehouse...</span>';
    setTimeout(() => {
